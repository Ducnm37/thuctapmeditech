...
Khai báo biến môi trường 
  <pre>$ . admin-openrc</pre>
Tạo user Neutron
  <pre>$ openstack user create --domain default --password-prompt neutron</pre>
...
$ openstack endpoint create --region RegionOne \
  network admin http://controller:9696</pre>
...
<li> Sử file /etc/neutron/neutron.conf với nội dung : </li>
<pre>
[database]
...
<li>Sửa file /etc/neutron/plugins/ml2/ml2_conf.ini với nội dung </li>
<pre>
[ml2]
...
[vxlan]
enable_vxlan = true
local_ip = 192.168.239.167
l2_population = true
 
[securitygroup]
enable_security_group = true
firewall_driver = neutron.agent.linux.iptables_firewall.IptablesFirewallDriver </pre>
<li> Sửa file /etc/neutron/l3_agent.ini với nội dung sau : </li>
<pre>[DEFAULT]
interface_driver = linuxbridge</pre>
<li>Sửa file /etc/neutron/dhcp_agent.ini với nội dung </li>
<pre>[DEFAULT]
...
<pre> 127.0.0.1       localhost compute1
 192.168.239.167     compute1
 192.168.239.166     controller   </pre>
...
server_proxyclient_address = $my_ip
novncproxy_base_url = http://192.168.239.166:6080/vnc_auto.html
...
| ID | Binary       | Host     | Zone | Status  | State | Updated At                 |
+----+--------------+----------+------+---------+-------+----------------------------+
|  7 | nova-compute | compute1 | nova | enabled | up    | 2018-05-09T09:11:02.000000 |
+----+--------------+----------+------+---------+-------+----------------------------+
# su -s /bin/sh -c "nova-manage cell_v2 discover_hosts --verbose" nova
</pre>

<pre>root@controller:~# openstack catalog list
+-----------+-----------+-----------------------------------------+
| Name      | Type      | Endpoints                               |
+-----------+-----------+-----------------------------------------+
...
CACHES = {
    'default': {
         ...
         'LOCATION': '192.168.239.166:11211',
    }
}
...
TIME_ZONE = "Asia/Ho_Chi_Minh"</pre>
...
<li>Tạo provider network : </li>
<pre>. admin-openrc
openstack network create  --share --external --provider-physical-network provider --provider-network-type flat provider</pre>
<li>Tạo subnet: </li>
<pre>openstack subnet create --network provider \
  --allocation-pool start=192.168.239.200,end=192.168.239.220 \
  --dns-nameserver 8.8.8.8 --gateway 192.168.239.2 \
  --subnet-range 192.168.239.0/24 provider</pre>
 <li>Tạo self-service network</li>
  <pre>. demo-openrc
  openstack network create selfservice</pre>
  <li>Tạo subnet: </li>
  <pre>openstack subnet create --network selfservice \
  --dns-nameserver 8.8.8.8 --gateway 10.10.10.1 \
  --subnet-range 10.10.10.0/24 selfservice</pre>
 <li> Tạo router </li>
 <pre>openstack router create router</pre>
 <li>Thêm self-service vào interface của router</li>
 <pre>neutron router-interface-add router selfservice </pre>
 <li>Set gateway của router là provider network để ra ngoài internet </li>
 <pre>neutron router-gateway-set router provider</pre>
 <li>Kiểm tra lại: </li>
 <pre>root@controller:~# ip netns
qdhcp-6c955319-b16f-49c2-9032-d37235cd5dca (id: 2)
qdhcp-c2dd0f92-8cfc-4a3e-bf25-fbb86ecd3f4b (id: 1)
qrouter-114c0742-1e74-461a-b643-4c610f847517 (id: 0)
</pre>
<pre>root@controller:~# neutron router-port-list router
+--------------------------------------+------+----------------------------------+-------------------+----------------------------------------------------------------------------------------+
| id                                   | name | tenant_id                        | mac_address       | fixed_ips                                                                              |
+--------------------------------------+------+----------------------------------+-------------------+----------------------------------------------------------------------------------------+
| 199d605e-e7c7-49f4-be47-f6bca41b9705 |      |                                  | fa:16:3e:1a:92:dc | {"subnet_id": "eaea9a0f-1065-4826-a1e6-e9f6a1b67bf2", "ip_address": "192.168.239.237"} |
| 777c2679-2766-4d3f-ba0f-6548a1c90e96 |      | f069dc7de84e4b3fa1e3268730ef6a24 | fa:16:3e:8d:79:f1 | {"subnet_id": "fc03ad2e-ea54-4f56-9206-085b9ae62e2c", "ip_address": "10.10.10.2"}      |
+--------------------------------------+------+----------------------------------+-------------------+----------------------------------------------------------------------------------------+
root@controller:~#</pre>
<li>Thêm security group rules cho phép ping và ssh </li>
